...
${i===0?'Today':new
Date(w.daily.time[i]).toLocaleDateString('en',{weekday:'short'})}<br>
...
console.log('Got coordinates:', loc.latitude, loc.longitude);
const w=await getWeather(loc.latitude,loc.longitude);
console.log('Got weather data:', w);
updateCurrent(loc,w);
updateHourly(w);
updateDaily(w);
